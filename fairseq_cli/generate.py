...
def _main(args, output_file):
    ...
    # load model and arg from checkpoint and setup task 
    logger.info('loading model(s) from {}'.format(args.path))
    models, args, task = checkpoint_utils.load_model_ensemble(
        utils.split_paths(args.path),
        arg_overrides=vars(args),
        task=None,
    ...
    # if hasattr(tgt_dict, "blank_index"): extra_symbols_to_ignore.append(tgt_dict.blank_index)
    # if hasattr(tgt_dict, "mask_index"): extra_symbols_to_ignore.append(tgt_dict.mask_index)
    ...
    for sample in progress:
        ...
        for i, sample_id in enumerate(sample['id'].tolist()):
            ...
            target_tokens = None
            if has_target:
                # target_tokens = _ctc_postprocess(sample['target'][i, :])
                target_tokens = utils.strip_pad(sample['target'][i, :], tgt_dict.pad()).int().cpu()
                target_tokens = utils.strip_pad(target_tokens, tgt_dict.pad()).int().cpu()
            ...
            for j, hypo in enumerate(hypos[i][:args.nbest]):
                hypo_tokens, hypo_str, alignment = utils.post_process_prediction(
                    # hypo_tokens=_ctc_postprocess(hypo['tokens'].int().cpu()),
