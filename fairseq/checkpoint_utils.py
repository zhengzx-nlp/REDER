...
def load_model_ensemble_and_task(filenames, arg_overrides=None, task=None, strict=True, suffix=''):
    ...
    for n, filename in enumerate(filenames):
        ...
        if (
            hasattr(args, 'iter_decode_with_external_reranker')
            and len(filenames) > 1
            and n == len(filenames) - 1
        ):
            if getattr(args, 'inference_direction', 'forward') == 'reverse':
                args.source_lang, args.target_lang = args.target_lang, args.source_lang
            at_task = tasks.setup_task(args)
            model = at_task.build_model(args)
