...
def average_checkpoints(inputs):
    ...
    for k, v in params_dict.items():
        averaged_params[k] = v
        if isinstance(averaged_params[k], torch.BoolTensor):
            continue
...
def last_n_checkpoints(paths, n, update_based, best_based, upper_bound=None):
    assert len(paths) == 1
    path = paths[0]
    if update_based:
...
def main():
    ...
    if num is not None:
        args.inputs = last_n_checkpoints(
            args.inputs, num, is_update_based, is_best_based, upper_bound=args.checkpoint_upper_bound,
        )
